...
test.describe('Complete Segmentation Workflow', () => {
  ...
  const testImagePaths = [
    path.join(__dirname, '../../public/lovable-uploads/026f6ae6-fa28-487c-8263-f49babd99dd3.png'),
    path.join(__dirname, '../../public/lovable-uploads/19687f60-a78f-49e3-ada7-8dfc6a5fab4e.png'),
    path.join(__dirname, '../../public/lovable-uploads/8f483962-36d5-4bae-8c90-c9542f8cc2d8.png')
  ];
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const isLoggedIn = await page.locator('[data-testid="user-menu"], .user-menu, [aria-label*="user"], [aria-label*="profile"]').isVisible();
    ...
    if (!isLoggedIn) {
      ...
      const consentCheckbox = page.getByRole('checkbox', { name: /terms|consent/i }).first();
  ...
  test('should complete full segmentation workflow with HRNet model', async ({ page }) => {
    ...
    await expect(page.getByText(/project.*created|success/i)).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByText(/upload.*complete|success/i)).toBeVisible({ timeout: 15000 });

    // Step 4: Start segmentation process
    const segmentButton = page.getByRole('button', { name: /segment|analyze|process/i }).first();
    ...
    const thresholdSlider = page.locator('input[type="range"], .threshold-slider');
    if (await thresholdSlider.isVisible()) {
      await thresholdSlider.fill('0.5');
    }

    // Step 6: Start processing
    await page.getByRole('button', { name: /start|process|segment|analyze/i }).click();

    // Wait for processing to complete
    await expect(page.getByText(/processing|analyzing|in progress/i)).toBeVisible({ timeout: 5000 });
    await expect(page.getByText(/complete|finished|done|success/i)).toBeVisible({ timeout: 60000 });

    // Step 7: Navigate to segmentation editor
    const editButton = page.getByRole('button', { name: /edit|open.*editor|view.*results/i }).first();
    await editButton.click();

    // Wait for editor to load
    await expect(page).toHaveURL(/.*\/segmentation.*/);
    await expect(page.locator('canvas, .canvas-container, .segmentation-canvas')).toBeVisible({ timeout: 10000 });

    // Step 8: Verify segmentation results are loaded
    await expect(page.getByText(/polygon|segment|object/i).first()).toBeVisible({ timeout: 10000 });
    ...
    const selectedPolygonIndicator = page.locator('.selected-polygon, .polygon-selected, [data-selected="true"]');
    ...
    if (await exportButton.isVisible()) {
      ...
      await expect(page.getByText(/coco|json|excel/i)).toBeVisible({ timeout: 5000 });
  ...
  test('should test all three ML models (HRNet, ResUNet Small, ResUNet Advanced)', async ({ page }) => {
    ...
    for (const modelName of models) {
      ...
      await page.getByRole('button', { name: /create.*project|new.*project/i }).click();
      await page.getByLabel(/project name|name/i).fill(`${testProject.name} - ${modelName}`);
      ...
      // Start segmentation
      await page.getByRole('button', { name: /segment|analyze|process/i }).first().click();
      ...
      await expect(page.getByText(/processing|analyzing/i)).toBeVisible({ timeout: 5000 });
      await expect(page.getByText(/complete|finished|done|success/i)).toBeVisible({ timeout });

      // Verify results exist
      const editButton = page.getByRole('button', { name: /edit|open.*editor|view.*results/i }).first();
      if (await editButton.isVisible()) {
        await editButton.click();
        await expect(page.locator('canvas, .canvas-container')).toBeVisible({ timeout: 10000 });
  ...
  test('should handle different threshold values', async ({ page }) => {
    ...
    for (const threshold of thresholds) {
      ...
      await page.getByRole('button', { name: /create.*project|new.*project/i }).click();
      await page.getByLabel(/project name|name/i).fill(`${testProject.name} - Threshold ${threshold}`);
      await page.getByLabel(/description/i).fill(`Testing threshold ${threshold}`);
      await page.getByRole('button', { name: /create|save/i }).click();

      await expect(page.getByText(/project.*created|success/i)).toBeVisible({ timeout: 10000 });
      
      // Navigate and upload
      await page.getByText(`${testProject.name} - Threshold ${threshold}`).click();
      ...
      // Set threshold
      const thresholdInput = page.locator('input[type="range"], input[type="number"]').first();
      ...
      if (await editButton.isVisible()) {
        ...
        const polygonElements = page.locator('.polygon, [data-polygon], .segment');
  ...
  test('should handle WebSocket real-time updates during processing', async ({ page }) => {
    ...
    for (const statusPattern of statusUpdates) {
      await expect(page.getByText(statusPattern)).toBeVisible({ timeout: 30000 });
    }
  ...
  test('should handle error scenarios gracefully', async ({ page }) => {
    ...
    if (!fs.default.existsSync(invalidFilePath)) {
      fs.default.writeFileSync(invalidFilePath, 'This is not an image file - testing invalid upload');
    }
    ...
    if (await fileInput.isVisible()) {
      try {
        ...
        await expect(page.getByText(/invalid.*file|unsupported.*format|error/i)).toBeVisible({ timeout: 5000 });
      } catch (error) {
    ...
    if (await uploadInput.isVisible()) {
      ...
      if (await cancelButton.isVisible({ timeout: 5000 })) {
        ...
        await expect(page.getByText(/cancelled|stopped|aborted/i)).toBeVisible({ timeout: 10000 });
      }
    }
  });
  ...
  test('should maintain performance with large images', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /create.*project|new.*project/i }).click();
    await page.getByLabel(/project name|name/i).fill('Performance Test Project');
    await page.getByLabel(/description/i).fill('Testing performance with large images');
    await page.getByRole('button', { name: /create|save/i }).click();
    ...
    const fileInput = page.locator('input[type="file"]').first();
    await fileInput.setInputFiles(testImagePaths[0]);
    await expect(page.getByText(/upload.*complete|success/i)).toBeVisible({ timeout: 20000 });
    ...
    await expect(page.getByText(/processing|analyzing/i)).toBeVisible({ timeout: 5000 });
    await expect(page.getByText(/complete|finished|done|success/i)).toBeVisible({ timeout: 120000 });
    ...
    if (await editButton.isVisible()) {
      ...
      await editButton.click();
      await expect(page.locator('canvas, .canvas-container')).toBeVisible({ timeout: 15000 });
  ...
  test('should save and restore session state', async ({ page }) => {
    ...
    await expect(page.getByText('Session Test Project')).toBeVisible({ timeout: 10000 });
