...
export function setup() {
  ...
  if (registerResponse.status === 201 || registerResponse.status === 409) {
    ...
    if (loginResponse.status === 200) {
      ...
      const projectResponse = http.post(`${BASE_URL}/api/projects`, projectPayload, {
        headers: {
          'Content-Type': 'application/json',
...
function testProjectsAPI(token) {
  ...
  const createResponse = http.post(`${BASE_URL}/api/projects`, projectPayload, { headers })
  ...
  if (createResponse.status === 201) {
    ...
    const updateResponse = http.put(`${BASE_URL}/api/projects/${projectId}`, updatePayload, { headers })
