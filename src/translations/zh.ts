export default {
  ...
  landing: {
    ...
    about: {
      ...
      supportText: '如果您愿意为本项目提供资金支持，请写信至',
    },
    acknowledgments: {
      badge: '致谢',
      title: '特别感谢',
  ...
  images: {
    ...
    upload: {
      ...
      op: {
        interrupted: '页面刷新导致上传中断',
        uploadingFile: '正在上传（{{index}}/{{total}}）',
        processingFile: '正在处理（{{index}}/{{total}}）',
        completedSummary: '上传完成：成功 {{success}} 个，失败 {{failed}} 个',
        preparing: '准备上传 {{count}} 个文件...',
        uploadingVideo: '正在上传视频 {{index}}/{{total}}',
        processingOnServer: '正在服务器上处理',
        uploadedWithFailures: '已上传 {{uploaded}} 个，失败 {{failed}} 个',
        uploadedOk: '{{count}} 个文件上传成功',
        uploadingFiles: '正在上传 {{count}} 个文件...',
        countUploaded: '已上传 {{count}} 个',
        countFailed: '失败 {{count}} 个',
        countCancelled: '已取消 {{count}} 个',
        noFiles: '未处理任何文件',
        retryingChunk: '正在重试分块 {{index}}（第 {{attempt}} 次）',
        processingChunk: '正在处理第 {{index}}/{{total}} 个分块',
        chunksDone: '已完成 {{done}}/{{total}} 个分块',
        uploadingChunk: '正在上传第 {{index}}/{{total}} 个分块',
        persistingOriginal: '正在保存原始文件',
        extractingFramesStart: '正在提取帧',
    ...
    modelSelection: {
      models: {
        ...
        microtubule: {
          ...
          description:
            '面向 IRM 微管延时实验的实例分割。nnU-Net ResEnc-M 网络预测纤维前景，再由曲率约束实例化算法将其分离为单条中线，每个交叉点都在 0.25 rad/px 的硬性曲率上限下求解。完全基于合成图像训练，无需任何人工标注。约 0.6 秒/帧；本平台唯一原生输出折线的模型。',
        },
        microcapsule: {
          name: 'Microcapsule',
          description:
  ...
  docs: {
    ...
    gettingStarted: {
      ...
      accountSteps: {
        ...
        step4: '在「设置」中选择界面语言与主题 — 分割模型在每个项目中单独设置',
      },
    ...
    modelSelection: {
      ...
      models: {
        ...
        microtubule: {
          name: '微管（SPARSE35）',
          inferenceTime: '每帧约 0.6 秒 · 阈值固定为 0.98，用户不可更改',
      ...
      howToSelect: '如何选择模型',
      selectionSteps: {
        step1: '打开项目 — 模型就在顶部，紧邻项目类型的右侧',
        step2: '选择器只提供该项目类型可运行的模型，并默认选中最准确的一个',
        step3: '选择图像后点击分割；使用的是该项目的模型',
        step4:
          '每个模型都带有自己的检测阈值，在验证时即已固定 — 单次运行无需调整',
        step5: '若是多通道视频，请指定模型应读取哪个通道',
        step6: '只有项目所有者可以更改模型；协作者使用所有者的选择进行分割',
      },
      thresholdNote: '检测阈值按模型固定。',
      thresholdNoteText:
        '界面中没有阈值控件：每个模型都使用其验证时确定的阈值，微管模型为 0.98。调低阈值并不会找出更多真实对象，只会找出更多证据薄弱的结果；而在非 IRM 通道上，无论怎样设置，微管模型的输出都与图像内容无关。若检出偏少，请检查输入通道。',
      tip: '提示：',
      tipText:
    ...
    segmentationProcess: {
      ...
      processingTimes:
        '创面模型每幅约 0.03 秒，球体模型约 0.2–0.4 秒，而微管模型每帧约 0.6 秒 — 因为把纤维一根根分离开才是耗时的部分。',
    },
    ...
    sharedProjects: {
      ...
      features: {
        collaborative:
          '协作访问：协作者可以查看、编辑标注、运行分割、导出并将项目标记为已审核 — 但不能重命名项目，也不能更改其类型或分割模型，这些仅限所有者',
        emailInvite:
    ...
    troubleshooting: {
      ...
      items: {
        ...
        noDetections: {
          symptom: '模型几乎没有检出结果',
          cause:
            '请先检查对比度与项目类型 — 阈值在任何地方都无法调整，每个模型使用其验证时确定的取值。如果该项目类型允许选择模型，更准确的模型可能有帮助；如果检测结果与图像内容无关，则说明分割的是错误的通道。',
        },
        wrongChannel: {
  ...
  admin: {
    ...
    usersTitle: '已注册用户',
    usersDescription:
      '选择一位用户以其身份登录，用于调试和技术支持。每次会话都会记录到审计日志中。',
    searchPlaceholder: '按邮箱或用户名搜索',
    columnUsername: '用户名',
    columnProjects: '项目',
    columnRegistered: '注册时间',
    badgeAdmin: '管理员',
    logInAs: '以此身份登录',
    cannotImpersonateSelf: '这是您自己的账户。',
    cannotImpersonateAdmin: '无法以其他管理员身份登录。',
    impersonateFailed: '无法以该用户身份登录。',
    loadUsersFailed: '无法加载用户列表。',
    noUsersFound: '没有符合此搜索条件的用户。',
    pageOf: '第 {{page}} 页，共 {{total}} 页',
    nextPage: '下一页',
    notAuthorizedTitle: '需要管理员权限',
    notAuthorizedDescription: '此页面仅对平台管理员开放。',
    impersonationBannerTitle: '技术支持会话。',
    impersonationBannerViewing: '您正在查看以下账户',
    impersonationBannerSignedInAs: '— 当前登录身份为',
    returnToUserList: '返回用户列表',
    stopImpersonationFailed: '无法结束技术支持会话。',
  },
};
