...
export const useVertexDrag = (
  ...
  const handleVertexDrag = useCallback((
    ...
    return false;
  }, [segmentation, setSegmentation, zoom, offset, vertexDragState]);
  ...
  const handleVertexClick = useCallback((
    ...
    const xSafe = typeof x === 'number' ? x.toFixed(2) : 'undefined';
    const ySafe = typeof y === 'number' ? y.toFixed(2) : 'undefined';
    console.log(`handleVertexClick: Checking vertex click at client: (${clientX}, ${clientY}), Canvas: (${canvasX}, ${canvasY}), Image: (${xSafe}, ${ySafe})`);
    ...
    for (const polygon of segmentation.polygons) {
      for (let i = 0; i < polygon.points.length; i++) {
        ...
        if (isNearVertex(canvasX, canvasY, point, 15)) {
          const pointXSafe = typeof point.x === 'number' ? point.x.toFixed(2) : 'undefined';
          const pointYSafe = typeof point.y === 'number' ? point.y.toFixed(2) : 'undefined';
          console.log(`Clicked on vertex at (${pointXSafe}, ${pointYSafe})`);
    ...
    return false;
  }, [segmentation, setSelectedPolygonId, isNearVertex, zoom, offset, vertexDragState]);
