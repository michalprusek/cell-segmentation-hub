...
interface EditorHeaderProps {
  ...
  /** Persist the current frame. Resolves `true` only when the edits actually
   *  reached the server — the leave prompt navigates on nothing weaker. */
...
const EditorHeader = ({
  ...
  // Where the user asked to go, held while the unsaved-changes prompt is up.
  // `null` closes the dialog, so this doubles as its open state.
  const [pendingDestination, setPendingDestination] = useState<string | null>(
    null
  );
  const [isSavingBeforeLeave, setIsSavingBeforeLeave] = useState(false);
  const [saveBeforeLeaveFailed, setSaveBeforeLeaveFailed] = useState(false);

  // startTransition is kept from the original handlers: it stops React 18 from
  // blocking the route change behind the editor's heavy re-render, which is
  // what used to freeze navigation after a segmentation.
  ...
  // Ask before leaving with unsaved edits. This used to navigate FIRST and
  // then race a background save against a 3 s timeout — but the editor's
  // unmount effect calls `abortAll()`, which aborts the very `manual-save`
  // signal that save runs on, so the edits were routinely dropped and the
  // only trace was a `logger.warn`.
  // Unsaved edits ALWAYS prompt, `onSave` or not. Falling through to
  // `leave()` when the handler is missing would discard them with no prompt at
  // all — the one outcome this component exists to prevent — so the dialog
  // opens either way and simply drops its Save button when it cannot save.
  const requestLeave = useCallback(
    (destination: string) => {
      if (hasUnsavedChanges) {
        setSaveBeforeLeaveFailed(false);
        setPendingDestination(destination);
        return;
      }
      leave(destination);
    },
    [hasUnsavedChanges, leave]
  );

  const handleBackClick = useCallback(() => {
    requestLeave(`/project/${projectId}`);
  ...
  const handleDismissLeave = useCallback(() => {
  ...
  }, [pendingDestination, leave]);

  const handleSaveAndLeave = useCallback(async () => {
    const destination = pendingDestination;
    if (!destination || !onSave) return;

    setIsSavingBeforeLeave(true);
    setSaveBeforeLeaveFailed(false);
    let saved = false;
    try {
      // Only an explicit `true` counts. A save that rejected, was aborted, or
      // simply had no handler must not be reported as persisted — navigating
      // on it is exactly the data loss this dialog exists to stop.
      saved = (await onSave()) === true;
    } catch {
      saved = false;
    } finally {
    ...
    if (!saved) {
      setSaveBeforeLeaveFailed(true);
      return;
    }
    setPendingDestination(null);
    leave(destination);
  }, [pendingDestination, onSave, leave]);
  ...
  return (
    <>
      ...
      <UnsavedChangesDialog
        ...
        onOpenChange={open => {
          if (!open) handleDismissLeave();
        }}
        onSaveAndLeave={onSave ? () => void handleSaveAndLeave() : undefined}
        onDiscardAndLeave={handleDiscardAndLeave}
        isSaving={isSavingBeforeLeave}
        saveFailed={saveBeforeLeaveFailed}
      />
    </>
  );
};
