...
import { shouldRenderVertices, getDecimatedVertices } from '@/lib/polygonOptimization';
...
const PolygonVertices = React.memo(({
  ...
  const shouldShowVertices = shouldRenderVertices(zoom, isSelected, isHovered);
  ...
  const visibleVertices = React.useMemo(() => {
    ...
    let verticesWithIndices = decimated.map(point => {
  ...
  if (!shouldShowVertices || points.length === 0 || visibleVertices.length === 0) {
    return null;
  }
  ...
  return (
    <g className="polygon-vertices">
      {visibleVertices.map(({ point, originalIndex }) => {
        const isVertexHovered = hoveredVertex?.polygonId === polygonId && hoveredVertex?.vertexIndex === originalIndex;
  ...
  const shouldShowPrev = shouldRenderVertices(prevProps.zoom, prevProps.isSelected, prevProps.isHovered);
  const shouldShowNext = shouldRenderVertices(nextProps.zoom, nextProps.isSelected, nextProps.isHovered);
