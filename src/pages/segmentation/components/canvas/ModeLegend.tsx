...
const ModeLegend: React.FC<ModeLegendProps> = ({
  ...
  const getCurrentMode = () => {
    if (editMode) return { label: 'Create Polygon', icon: Plus, color: 'purple' };
    if (slicingMode) return { label: 'Slice Polygon', icon: Scissors, color: 'red' };
    if (pointAddingMode) return { label: 'Add Points', icon: Edit3, color: 'emerald' };
    if (deleteMode) return { label: 'Delete Polygon', icon: Trash2, color: 'orange' };
    return null;
  };
  ...
  return (
    <AnimatePresence>
      {currentMode && (
        ...
        >
          <div className={`flex items-center space-x-2 px-3 py-2 rounded-lg shadow-lg border backdrop-blur-sm transition-all duration-200 ${
