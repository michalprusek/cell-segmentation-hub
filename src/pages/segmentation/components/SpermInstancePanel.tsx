 ...
 *
 * Measured with 311 polylines: 6531 `calculatePolylineLength` calls over 21
 * renders where 311 would do.
 *
 * The memo hits because every prop the layout passes is reference-stable across
 * a pan: `handleSelectPolygon` is a `useCallback([])` in `usePolygonHandlers`,
 * `onPartClassChange`/`onInstanceIdChange` are `useState` setters, and
 * `editor.polygons` is replaced by `setPolygons`, never mutated.
 */
export default React.memo(SpermInstancePanel);
