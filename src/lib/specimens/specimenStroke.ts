 ...
 * a preview tile has no selection — and `specimenStroke.test.ts` pins the two
 * together.
 */
...
export interface SpecimenOutline {
  ...
  readonly t?: 'i';
  /** The polygon's `partClass` (`core`, `head`, `midpiece`, `tail`,
   *  `neurite`, `soma`). Absent when it has none. */
  readonly c?: string;
...
const SPERM_PARTS: Record<string, string> = {
  head: '#22c55e',
  midpiece: '#f59e0b',
  tail: '#06b6d4',
};
...
export function specimenStroke(outline: SpecimenOutline): string {
  ...
  const part = outline.c ? SPERM_PARTS[outline.c] : undefined;
  if (part) return part;
