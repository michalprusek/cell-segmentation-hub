...
const AccountSection = () => {
  ...
  const showPasswordMatchIndicator = passwordData.newPassword.length > 0 && passwordData.confirmPassword.length > 0;
  ...
  const handleSaveAccount = async (e: React.FormEvent) => {
    ...
    if (passwordData.newPassword.length < 6) {
      toast.error(t('settings.passwordTooShort') || 'Heslo musí mít alespoň 6 znaků');
      return;
    }
    ...
    try {
      ...
      toast.success(t('settings.passwordChanged') || 'Heslo bylo úspěšně změněno');
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input 
                ...
                disabled={isLoading}
                className={showPasswordMatchIndicator ? (passwordsMatch ? 'border-green-500' : 'border-red-500') : ''}
                aria-invalid={showPasswordMatchIndicator ? !passwordsMatch : false}
                aria-describedby={showPasswordMatchIndicator ? "password-match-status" : undefined}
              />
                ...
                >
                  {passwordsMatch ? (
                    <>
                      ...
                      <span>{t('settings.passwordsMatch') || 'Hesla se shodují'}</span>
                    </>
                  ) : (
                    <>
                      <X className="w-4 h-4" aria-hidden="true" />
                      <span>{t('settings.passwordsDoNotMatch') || 'Hesla se neshodují'}</span>
                    </>
                  )}
                </div>
              )}
        ...
        <div className="flex justify-end">
          <Button type="submit" disabled={isLoading}>
            {isLoading ? (t('settings.changingPassword') || 'Měním heslo...') : (t('settings.changePassword') || 'Změnit heslo')}
          </Button>
        </div>
