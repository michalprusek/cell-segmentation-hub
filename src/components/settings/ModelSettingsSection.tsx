...
const ModelSettingsSection = () => {
  ...
  const getSizeIcon = (size: string) => {
    switch (size) {
      ...
      case 'medium': return <Cpu className="h-4 w-4" />;
      case 'large': return <Target className="h-4 w-4" />;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-medium mb-4">{t('settings.modelSelection')}</h3>
          ...
          <div className="space-y-4">
            {availableModels.map((model) => (
              <div key={model.id} className="flex items-center space-x-4">
                ...
                <Card 
                  className="flex-1 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800"
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 text-center">
            {t('settings.currentThreshold')}: {Math.round(confidenceThreshold * 100)}%
          </div>
        </div>
      </div>
