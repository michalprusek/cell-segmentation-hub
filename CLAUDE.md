...
- **Kymograph trajectory intensities from before 2026-09-01 are not comparable with later ones.** `intensity_signal` / `intensity_background` / `intensity_minus_background` in `velocity_metrics.xlsx` changed for EVERY trajectory when the metric moved onto the shared per-microtubule code (`models/mt_measure.py`, the same file the export and the essays batch use). The old 1-D estimator read a `width`-column band out of each row plus two `width`-column background blocks offset `bg_gap = 2` columns either side, pooled over the trajectory's entire temporal extent, and took `numpy.median`. It is now the microtubule's own region and statistics: an ImageJ `Roi.convertLineToArea` band measured PERPENDICULAR to the trajectory, a background ring `dilate(band, intensity_width * intensity_bg_margin)` **minus the union of every trajectory's band**, and ImageJ's histogram tie-rule median. A neighbouring streak can therefore no longer be counted as background — measured on real production kymographs, 17 of 47 trajectories of container `4972cad8` (300 frames, 640 nm, `intensity_width` 5) had a contaminated background, up to 18.5 % of its pixels, and 17 of 41 on a denser container, up to 42.7 %. Across those two sets `intensity_background` moved by a median of +0.3 % / +1.7 % (max 45.6 %) and `intensity_minus_bg` by −10.1 % / −4.1 % (max 46.3 %). The ring is **isotropic on axes that are not the same quantity** — rows are frames, columns are arc-length pixels, so `dilate(band, r)` means ±r frames AND ±r columns. That is deliberate (it is the microtubule region, and an anisotropic variant would need an acquisition-dependent frames-per-pixel scale) and it is still strictly MORE local in time than the estimator it replaced, which pooled the whole trajectory. `intensity_bg_margin` (`KymographRequest`, default 2.0, mirroring `MTMetricsRequest.margin_multiplier`) is the knob; nothing on the Node side sets it yet. Do not try to restore the old numbers.
