...
Production runs on `docker-compose.production.yml` with `.env.production`. Single-stack deployment (no more blue-green); containers are named `spheroseg-*` (frontend/backend/ml/postgres/redis/nginx). Database: `spheroseg`. URL: `https://spherosegapp.utia.cas.cz`. Test account: `12bprusek@gym-nymburk.cz` / `***REMOVED-CREDENTIAL***`.
...
- **The GPU cap is now sized for a model that is gone.** Everything in the paragraph below was measured against microtubule **v7**. v5H replaced it on 2026-08-17 and peaks at **0.73 GiB**, flat across 1024² and 2048² (it tiles at 512² instead of running a ViT over the whole frame), so the 16.36 GiB working set no longer exists and `ESSAYS_GPU_MEM_FRACTION=0.75` reserves ~17 GiB for a model needing under 1. Re-measure a real batch run before lowering it — the failure mode of getting this wrong was 255 lost wells — but there is a large amount of VRAM to hand back to interactive segmentation here. The reasoning below is kept because it is the reasoning to repeat when re-tuning.
- **Set the cap ABOVE the model's working set (16.36 GiB on a 2048² well **for v7**), never below.** `ESSAYS_GPU_MEM_FRACTION` is `0.75` (17.67 GiB of the 24 GB A5000) and `ESSAYS_GPU_MIN_FREE_GB` is `17` since 2026-08-13. It was 0.6 / 13 GB, i.e. the cap sat _under_ the working set, so torch hit the ceiling on every position, released ~3.7 GiB of cached blocks back to the driver and had to re-win them from a card shared with `ml` and Maptimize. Losing that race is an `OutOfMemoryError`, which is why one folder lost 255 wells in one run and 68 in the next. The 16.36 GiB figure is flat across caps of 16.49 / 17.67 / 20.02 GiB — demand does not grow into the ceiling — so raising the cap past ~0.75 buys nothing and only takes room interactive segmentation needs. A cap below the working set protects nobody: it hands other tenants memory the batch immediately tries to take back.
...
- **DB password literal `***REMOVED-CREDENTIAL***`** is a fixed credential string (NOT a DB name reference). Database itself is `spheroseg` since 2026-05-15. Rotating the password means changing the live user's `PG_PASSWORD` first.
