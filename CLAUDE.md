...
- **The trajectory intensity floor (`min_intensity_minus_bg`) is an ABSOLUTE number, and that is only sound because of where it is applied.** Frames are decoded at native bit depth (`_decode_rows` never converts a 16-bit frame to 8-bit) and `tracks_intensity` measures on that raw matrix, so `intensity_minus_bg` is in camera counts; the min/max normalisation later in the endpoint exists only to paint the PNG. A floor on the DETECTOR could not be absolute — KymoButler consumes `preprocess_array`'s row-normalised copy, whose units are arbitrary — so this can only drop trajectories the detector already found. It runs AFTER `tracks_intensity` (the background ring is the union of every trajectory's band, so filtering first would measure each survivor as if its dropped neighbours were empty field) and BEFORE `render_overlay` and `flag_bright_outliers`, exactly like the velocity cut-off, so overlay = table = exported CSV. It is **polarity-aware**: on a dark-on-bright kymograph the signal sits BELOW the background, `intensity_minus_bg` is negative, and comparing raw would hide every particle on the movie. A `None` intensity is KEPT — a failed measurement is not evidence of a dim trajectory. **It is NOT comparable between channels**: measured on two production containers, `intensity_minus_bg` runs 9-51 counts on 488 nm and 228 on 640 nm of the SAME movie, because subtracting the background removes the offset and not the scale (dye, exposure, gain). `snr` is the dimensionless alternative and is already per track. Default 0 = off, and the field is omitted from the ML body at 0, so an un-recreated ml container cannot 422 — **deploy ml before the backend**, as always. It is part of the kymograph cache key; without that, nudging it would be served the previous threshold's tracks and the control would look inert.
