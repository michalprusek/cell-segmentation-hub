...
async function checkData() {
  try {
    ...
    segmentations.forEach(seg => {
      ...
      console.log(`  - Image ${seg.imageId?.slice(0, 8)}: ${polygonCount} polygons, ${seg.imageWidth}x${seg.imageHeight}`);
    });
    
    // Now let's check if images with 'segmented' status have corresponding segmentation data
    const segmentedImages = images.filter(img => img.segmentationStatus === 'segmented');
    console.log('🔍 Images marked as segmented:', segmentedImages.length);
    
    for (const img of segmentedImages.slice(0, 3)) {  // Check first 3 segmented images
      const segmentation = segmentations.find(seg => seg.imageId === img.id);
      console.log(`  📸 ${img.name?.slice(0, 30)} (${img.id.slice(0, 8)}):`, 
                  segmentation ? `${JSON.parse(segmentation.polygons).length} polygons` : 'NO SEGMENTATION DATA');
    }
