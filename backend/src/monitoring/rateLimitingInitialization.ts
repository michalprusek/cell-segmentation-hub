  ...
  async createLimiter(
    ...
    if (client) {
      try {
        ...
        logger.warn(
          `Failed to create Redis store for rate limiter '${tierName}', using memory store`,
          error
        );
      }
    } else {
      logger.warn(
        `Rate limiter '${tierName}' using memory store (Redis not available)`
      );
    }
    ...
    logger.info(
      `Rate limiter '${tierName}' initialized: ${tier.max} requests per ${tier.windowMs}ms`
    );
    return limiter;
  }
...
export const rateLimiters = {
  ...
  getTierLimiter: (tier: string): Promise<RateLimitRequestHandler | null> =>
    rateLimitingSystem.getLimiter(tier),
  ...
  anonymous: (): Promise<RateLimitRequestHandler | null> =>
    rateLimitingSystem.getLimiter('anonymous'),
  authenticated: (): Promise<RateLimitRequestHandler | null> =>
    rateLimitingSystem.getLimiter('authenticated'),
  api: (): Promise<RateLimitRequestHandler | null> =>
    rateLimitingSystem.getLimiter('api'),
  auth: (): Promise<RateLimitRequestHandler | null> =>
    rateLimitingSystem.getLimiter('auth'),
  upload: (): Promise<RateLimitRequestHandler | null> =>
    rateLimitingSystem.getLimiter('upload'),
  admin: (): Promise<RateLimitRequestHandler | null> =>
    rateLimitingSystem.getLimiter('admin'),
};
