...
import { getQueueStatus, forceProcessQueue, getQueuedEmails } from '../../services/emailRetryService';
...
router.post('/send-direct', authenticate, async (req: Request, res: Response) => {
  try {
    const { to } = req.body;
...
If you received this email, the direct send configuration is working!`
    }, false); // allowQueue = false to force direct send
...
router.post('/force-queue-process', authenticate, async (req: Request, res: Response) => {
  try {
    logger.info('Manual queue processing requested via API', 'TestEmailRoutes');
...
router.get('/queue-emails', authenticate, async (req: Request, res: Response) => {
  try {
