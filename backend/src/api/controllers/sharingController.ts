 ...
 */
export const shareProjectByEmail = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const share = await SharingService.shareProjectByEmail(
        projectId,
        req.user.id,
        data
      );
      ...
      if (
        ...
        errorMessage.includes('already shared') ||
        errorMessage.includes('Cannot share')
      ) {
 ...
 */
export const shareProjectByLink = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    if (!req.user) {
      ResponseHelper.unauthorized(
        res,
    ...
    try {
      ...
      const share = await SharingService.shareProjectByLink(
        projectId,
        req.user.id,
        data
      );
 ...
 */
export const getProjectShares = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    if (!req.user) {
      ResponseHelper.unauthorized(
        res,
    ...
    try {
      ...
      logger.debug(
        'Starting getProjectShares access check',
        'SharingController',
        {
          userId: req.user.id,
          projectId,
      ...
      const shares = await SharingService.getProjectShares(
        projectId,
        req.user.id
      );
 ...
 */
export const revokeProjectShare = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    if (!req.user) {
      ResponseHelper.unauthorized(
        res,
    ...
    try {
      ...
      ResponseHelper.success(res, null, 'Share revoked successfully');
    } catch (error) {
      logger.error(
 ...
 */
export const getSharedProjects = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    if (!req.user) {
      ResponseHelper.unauthorized(
        res,
    ...
    try {
      const shares = (await SharingService.getSharedProjects(
        req.user.id
      )) as ShareWithDetailedProject[];

      logger.info(
        `Found ${shares?.length || 0} shared projects for user`,
        'SharingController',
        {
          userId: req.user.id,
 ...
 */
export const validateShareToken = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
 ...
 */
export const acceptShareInvitation = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    const { token } = req.params;
    if (!token) {
      ResponseHelper.badRequest(res, 'Token is required');
    ...
    try {
      const resultData = await SharingService.acceptShareInvitation(
        token,
        req.user?.id
      );
