...
describe('Database Integration Tests', () => {
  ...
  describe('Complex Queries', () => {
    ...
    it('should get user with all projects, images, and segmentations', async () => {
      ...
      const hasSegmentation = userWithData?.projects?.[0]?.images?.some(
        img =>
    ...
    it('should count images by processing status', async () => {
      ...
      const segmentedCount =
        statusCounts.find(s => s.segmentationStatus === 'segmented')?._count
          .segmentationStatus || 0;
      const pendingCount =
        statusCounts.find(s => s.segmentationStatus === 'pending')?._count
          .segmentationStatus || 0;
