...
class DatabaseOptimization {
  ...
  public getSlowQueryAnalysis(): {
    ...
    this.queryTimings.forEach((timings, queryName) => {
      const slowTimings = timings.filter(
        time => time > this.config.slowQueryThreshold
      );
      if (slowTimings.length > 0) {
        const averageTime =
          slowTimings.reduce((sum, time) => sum + time, 0) / slowTimings.length;
        analysis.push({
          queryName,
          averageTime,
  ...
  public getOptimizationRecommendations(): string[] {
    ...
    if (this.metrics.slowQueries > this.metrics.totalQueries * 0.1) {
      recommendations.push(
        'High number of slow queries detected. Consider adding database indexes.'
      );
    }

    if (this.metrics.averageQueryTime > 500) {
      recommendations.push(
        'Average query time is high. Review query complexity and indexing strategy.'
      );
    }
    ...
    if (slowQueries.length > 0) {
      ...
      recommendations.push(
        `Slowest query: ${topSlow.queryName} (avg: ${topSlow.averageTime.toFixed(2)}ms)`
      );
    }
