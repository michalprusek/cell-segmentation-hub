...
const ENV_VARIABLES: EnvVariable[] = [
  // Database
  {
    ...
    validator: value =>
      value.includes('postgresql://') || value.includes('sqlite:'),
  },
...
export function validateEnvironment(): ValidationResult {
  ...
  for (const envVar of ENV_VARIABLES) {
    ...
    if (!value && !envVar.defaultValue) {
      if (envVar.required) {
        errors.push(
          `Missing required environment variable: ${envVar.name} - ${envVar.description || 'No description'}`
        );
        missing++;
      } else {
        warnings.push(
          `Optional environment variable not set: ${envVar.name} - ${envVar.description || 'No description'}`
        );
      }
      continue;
    }
...
export function getNumericEnvVar(name: string, defaultValue: number): number {
  ...
  if (isNaN(parsed)) {
    logger.warn(
      `Invalid numeric value for ${name}: ${value}, using default: ${defaultValue}`,
      'EnvValidator'
    );
    return defaultValue;
  }
