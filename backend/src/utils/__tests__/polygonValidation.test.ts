...
describe('PolygonValidator', () => {
  describe('parsePolygonData', () => {
    test('should parse valid JSON string polygon data', () => {
      const validPolygons = [
        { id: '1', points: [{ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 5, y: 10 }] },
        { id: '2', points: [{ x: 20, y: 20 }, { x: 30, y: 20 }, { x: 25, y: 30 }] }
      ];
      const jsonString = JSON.stringify(validPolygons);
      
      const result = PolygonValidator.parsePolygonData(jsonString, 'test-context', 'test-image');
    ...
    test('should handle invalid JSON string gracefully', () => {
      ...
      const result = PolygonValidator.parsePolygonData(invalidJson, 'test-context', 'test-image');
    ...
    test('should handle null/undefined input', () => {
      const nullResult = PolygonValidator.parsePolygonData(null, 'test-context', 'test-image');
      const undefinedResult = PolygonValidator.parsePolygonData(undefined, 'test-context', 'test-image');
    ...
    test('should handle empty string', () => {
      const result = PolygonValidator.parsePolygonData('', 'test-context', 'test-image');
    ...
    test('should filter out invalid polygons', () => {
      const mixedPolygons = [
        { id: '1', points: [{ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 5, y: 10 }] }, // valid
        { id: '2', points: [{ x: 20, y: 20 }] }, // invalid - only 1 point
        { id: '3', points: [{ x: 'invalid', y: 30 }, { x: 25, y: 30 }] }, // invalid - non-numeric x
        { id: '4', points: [{ x: 40, y: 40 }, { x: 50, y: 40 }, { x: 45, y: 50 }] } // valid
      ];
