...
const configSchema = z
  .object({
    ...
    UPLOAD_DIR: z
      .string()
      .min(1, 'Upload directory cannot be empty')
      .default('./uploads'),
    EXPORT_DIR: z
      .string()
      .min(1, 'Export directory cannot be empty')
      .default('./exports'),
    MAX_FILE_SIZE: z
      .string()
      .transform(val => {
  ...
  .refine(data => {
    ...
    if (data.EMAIL_SERVICE === 'sendgrid') {
      if (!data.SENDGRID_API_KEY || data.SENDGRID_API_KEY.trim() === '') {
        throw new Error(
          'SENDGRID_API_KEY is required when EMAIL_SERVICE is sendgrid'
        );
      }
    ...
    if (data.NODE_ENV === 'production') {
      ...
      if (
        !data.SEGMENTATION_SERVICE_URL ||
        data.SEGMENTATION_SERVICE_URL.trim() === ''
      ) {
...
const parseConfig = (): ConfigType => {
  try {
    ...
    logger.error('Environment variables debug info:');
    // Show current environment
    logger.error(`NODE_ENV: ${process.env.NODE_ENV || 'undefined'}`);
    // Email configuration check
    logger.error(`FROM_EMAIL: ${process.env.FROM_EMAIL || 'undefined'}`);

    // Only log boolean presence, never expose secret details
    logger.error(
      `JWT_ACCESS_SECRET configured: ${!!process.env.JWT_ACCESS_SECRET}`
    );
    logger.error(
      `JWT_REFRESH_SECRET configured: ${!!process.env.JWT_REFRESH_SECRET}`
    );
