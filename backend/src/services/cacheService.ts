...
class CacheService {
  ...
  async invalidatePattern(pattern: string, options: CacheOptions = {}): Promise<number> {
    try {
      ...
      if (deletedCount > 0) {
        ...
        logger.info(`Invalidated ${deletedCount} cache entries matching pattern: ${fullPattern}`, 'Cache');
      }

      return deletedCount;
    } catch (error) {
      logger.error('Cache pattern invalidation failed:', error as Error, 'Cache');
      return 0;
    }
  }
  ...
  async warmCache<T>(
      ...
      const results = await Promise.allSettled(
        chunk.map(async (entry) => {
          try {
            ...
            logger.error(`Cache warming failed for key ${entry.key}:`, error as Error, 'Cache');
            return false;
          }
        })
      );
    ...
    logger.info(`Cache warming completed: ${success} success, ${failed} failed`, 'Cache');
