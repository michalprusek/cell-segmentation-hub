...
export function getScaleWarningMessage(scale: number): string {
  if (scale > SCALE_CONFIG.HIGH_SCALE_WARNING) {
    ...
    return (
      `High scale value detected: ${scale} um/pixel. This is higher than typical microscopy scales. ` +
  ...
  if (scale < SCALE_CONFIG.LOW_SCALE_WARNING) {
    return (
      `Low scale value detected: ${scale} um/pixel. This is lower than typical microscopy scales. ` +
...
function findClosestTypicalScale(scale: number): string {
  ...
  let closestDiff = Math.abs(
    scale - SCALE_CONFIG.TYPICAL_SCALES['4x_objective']
  );
  ...
  return closestKey
    .replace('_', ' ')
    .replace('objective', 'objective magnification');
}
