...
from .vision_module import get_candidates
...
def track_bidirectional(
    ...
    while np.sum(remaining) > STRADDLER_PIXEL_THRESHOLD and iteration < STRADDLER_MAX_ITERATIONS:
        ...
        if len(remaining_yx) == 0:
            break
        # VENDOR NOTE: upstream builds `KDTree(remaining_yx)` here and then
        # never passes it — `_make_track` below is called with the ORIGINAL
        # full-skeleton `kdtree`/`allyx_coords`. The build is therefore pure
        # waste, and not cheap: a KDTree over every still-untracked pixel, once
        # per iteration, up to STRADDLER_MAX_ITERATIONS = 500 times.
        #
        # It is deleted rather than wired up. Wiring it would change what the
        # straddler pass searches (remainder-only instead of the whole
        # skeleton), which is a real behaviour change — and the thresholds in
        # config.py were calibrated against upstream's published reference
        # counts WITH this pass behaving as it does here. Dropping a build whose
        # result is discarded cannot change any output.
